  ...
  <div v-if="showModal" class="modal">
    <div class="modal-content">
      ...
      <div v-if="modalMode === 'add'">
        ...
        <label class="col-md-6 col-12">Meeting End Time:</label>
      ...
      <div v-else>
        ...
        <p
          v-if="selectedEvent.participants && selectedEvent.participants.length"
        >
  ...
  methods: {
    ...
    async saveEvent() {
      ...
      if (title && startTime && endTime && this.userId) {
        ...
        try {
          ...
          console.log(
            `Event added to organization_calendar with ID: ${orgDocRef.id}`
          );

          // Save to user's personal calendar
          for (const participantId of event.participants) {
            const participantEventsRef = collection(
              db,
              "users",
              participantId,
              "meeting"
            ...
            console.log(
              `Event added to participant (userId: ${participantId}) calendar`
            );
          }
    ...
    async deleteEvent() {
      if (this.selectedEvent) {
        try {
          const participants = await this.fetchEventParticipants(
          ...
          for (const participantId of participants) {
            ...
            console.log(
              `Deleted event from participant (userId: ${participantId}) calendar`
            );
          }
